...
The current repository has completed only the SO101 VLA-JEPA action-baseline
path. It reuses the pretrained VLA representation, extracts latent and embodied
action tokens from the Qwen/V-JEPA token stream, and adapts those tokens to a
flow-matching action head that predicts continuous follower-arm action chunks.
...
default-off latent, transition, utility-proxy, or direct-expert losses. The
...
paths remain disabled by default and should be treated as research hooks rather
...
\texttt{lara\_so101\_utility\_pool.yaml}. The first is the main VLA action
baseline. The latter three enable default-off latent-action, direct-MoE, and
utility/pool scaffolds for controlled experiments, but they do not by
themselves constitute completed LARA evidence.
