...
MoE/router, direct action experts, and utility-proxy losses before predicting
continuous follower-arm action chunks.
...
residual or direct action experts, posterior-responsibility signals, an
...
paths are now default-on and should be treated as active research code rather
...
The primary metric is closed-loop success under matched active inference compute. Secondary metrics are return, sample-efficiency AUC, compute-success Pareto efficiency, policy-ranking Spearman $\rho$ and Kendall $\tau$, posterior-router KL, teacher mass@resident-pool, teacher mass@active-top-$k$, critical expert miss rate, top-$k$ route consistency, route regret, expert usage entropy, route-switch frequency, utility calibration error, train-time overhead, and inference latency. If the episode-level route pool is used, also report subset-retention curves: success versus percentage of resident experts retained, e.g., 100\%, 50\%, 25\%, and 12.5\% resident experts.
