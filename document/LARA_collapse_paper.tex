...
default-off latent, transition, utility-proxy, or direct-expert losses.
...
action head, residual or direct action experts, posterior-responsibility
signals, an episode-level resident-pool router, a chunk-level top-$k$ router
inside that pool, route diagnostics, and counterfactual-utility sidecar
plumbing. These paths remain disabled by default and should be treated as
research hooks rather than completed paper evidence. In particular, the current
repository does not yet provide a trained and validated MoE action head, a
validated two-level routing policy, real counterfactual utility labels, or
closed-loop SO101 results demonstrating resident-pool reuse and sparse expert
selection under matched compute.
