...
The current repository implements the SO101 VLA-JEPA action baseline: the
...
continuous follower-arm action chunk. The SO101 dataloader and action adapter
also propagate future-action validity masks so trajectory-end padding does not
supervise action, latent, transition, utility-proxy, or direct-expert losses.
For the optional MoE path, the dataloader can also return the episode's first
image so the episode-level pool router can condition on an encoded initial
context $h_1$ rather than reusing the current chunk context as a proxy.
Protocol tooling can summarize rollout JSON/JSONL records into matched-budget
rows, subset-retention curves, and compute-success Pareto flags, but it does
not itself produce real FLOP, latency, memory, or closed-loop success
measurements. It also includes route-sequence diagnostics for resident-pool
reuse across receding-horizon chunks, but those diagnostics are only meaningful
once attached to raw router-probability and route-mask traces from real
closed-loop rollouts. The deployment websocket server can optionally write
those raw route traces and server-side latency/VRAM measurements to JSONL, but
the repository still lacks completed closed-loop SO101 evaluation runs. A
strict evidence-audit mode can reject incomplete rollout records, which helps
prevent protocol summaries from being mistaken for completed paper evidence.
This is not yet the full LARA algorithm in this manuscript. In particular,
production-ready latent-action training, validated MoE action experts,
posterior-responsibility training, the episode-level pool router, the
chunk-level top-$k$ router inside the resident pool, and counterfactual utility
calibration remain future implementation work. Any current latent-action or
MoE/router code should be treated as default-off experimental scaffolding until
it has full training, diagnostic, matched-budget, and closed-loop validation.
