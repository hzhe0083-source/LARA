...
uses $M$ residual experts $\{\Delta E_m\}_{m=1}^M$:
\begin{equation}
 \hat{a}^{(m)}_{t:t+H-1}
 = a_{\mathrm{sh},t:t+H-1}+\Delta E_m(h_t,z_t^a).
...
\begin{equation}
 q(m|h_t,a_{t:t+H-1}) =
 \frac{\exp\left((\ell_{\mathrm{sh}}-\ell_m-\beta\|\Delta E_m(h_t,z_t^a)\|^2)/\tau_q\right)}
 {\sum_{j=1}^{M}\exp\left((\ell_{\mathrm{sh}}-\ell_j-\beta\|\Delta E_j(h_t,z_t^a)\|^2)/\tau_q\right)}.
...
\begin{equation}
 ...
 +\lambda_{\Delta}\|\Delta E_m(h_i,z_i^a)\|^2.
...
\State Predict residual corrections $\Delta E_m(h_t,z_t^a)$ and expert-corrected chunks $\hat{a}^{(m)}=a_{\mathrm{sh}}+\Delta E_m(h_t,z_t^a)$.
\State Compute posterior expert responsibilities from improvement over the shared loss, $q_{\mathrm{post}}(m|h_t,a_{t:t+H_p-1})$.
\State Update residual experts with responsibility-weighted loss $\mathcal{L}_{\mathrm{expert}}$ and residual norm/diversity diagnostics.
