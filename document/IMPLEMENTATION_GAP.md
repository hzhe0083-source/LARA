...
- Stage-2 MoE/router code was added behind `use_lara_moe`; it now has torch tests for resident-pool routing, resident-pool mask reuse across chunks, chunk top-k routing, per-expert action-loss posterior responsibility, routed direct-expert action output, and episode-level pool target aggregation, but it is not yet validated in a full training run.
