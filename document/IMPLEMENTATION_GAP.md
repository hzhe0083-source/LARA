...
- Independent validation dataloader support through `datasets.vla_eval_data`; evaluation metrics are mask-aware and split full prediction horizon from executable horizon.
- Configured world-model loss weight through `trainer.loss_scale.wm` with raw/weighted metrics, instead of a hard-coded multiplier.
...
- Stage-1 latent action head scaffold: posterior encoder, VQ codebook, action-chunk decoder/reconstruction loss, optional code-usage regularization, context-only prior, and optional execution/prediction boundary-state transition loss (`use_latent_action_head: false`, `lara_use_transition_head: false`).
...
- The latent-action head now includes an explicit action-chunk decoder and reconstruction loss over the `latent_action_horizon` target, with `future_action_mask` support and visible raw/weight/weighted loss metrics.
- Action-head auxiliary losses now expose raw, configured weight, and weighted variants for latent, transition, MoE, utility, balancing, and direct-expert losses so trainer logs do not hide multiple objectives inside a single router-loss scalar.
...
- Four explicit SO101 stage configs now exist: `scripts/config/lara_so101_baseline.yaml`, `scripts/config/lara_so101_latent_vq.yaml`, `scripts/config/lara_so101_moe_direct.yaml`, and `scripts/config/lara_so101_utility_pool.yaml`. The old `lara_so101_ft.yaml` remains as a baseline compatibility entry point.
...
- `scripts/prepare_lara_utility_training.py` turns real forced-expert rollout records into a validated utility sidecar plus a derived SO101 utility-training YAML with positive `lara_utility_loss_weight`, labeled-only sampling, and a ready-to-run `accelerate launch ... --config_yaml ...` command. This closes the artifact handoff from rollout logging to utility training, but it still depends on externally collected forced-expert outcomes and does not itself produce robot evidence.
...
- LIBERO100/MetaWorld LeRobot v3 dataloaders now emit `future_actions`, `current_state`, boundary-state targets, current Qwen images, and V-JEPA `video` tensors using `framework.vj2_model.num_frames` rather than the action horizon. Local LIBERO100 files pass the downloader preflight, but real-batch loading still requires the optional upstream `lerobot` dependency (`pip install -e '.[benchmark]'`) in the active environment before full benchmark smoke can run.
- `scripts/audit_lara_paper_readiness.py` now audits baseline-safe defaults, SO101 horizon/config contracts, counterfactual utility sidecars, closed-loop rollout protocol evidence, full SO101 training artifacts, and real robot evaluation artifacts. The training/eval artifacts are validated as structured JSON with required paper-stage flags, finite losses, checkpoint evidence, real-SO101/real-robot markers, route diagnostics, matched-compute metrics, and counterfactual utility evaluation fields. It intentionally reports the repo as not paper-ready while those required artifacts are missing or too weak.
