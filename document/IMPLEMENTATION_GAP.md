...
- Stage-2 MoE/router scaffold: residual experts, posterior responsibility head, chunk router, and episode pool router (`use_lara_moe: false`).
...
- Posterior responsibility training from demonstrated future action chunks.
- Episode-level pool router trained from trajectory-level posterior responsibility.
- Chunk-level top-k router constrained to the resident episode pool.
- Router distillation from posterior responsibility.
...
- Stage-2 MoE/router code was added behind `use_lara_moe`; it is not yet validated in a full training run.
...
- The MoE/router path is currently a Stage-2 skeleton and still needs route-quality diagnostics, expert-collapse checks, resident-pool evaluation, and closed-loop validation.
...
4. Validate the optional MoE/router path and add route-quality diagnostics.
