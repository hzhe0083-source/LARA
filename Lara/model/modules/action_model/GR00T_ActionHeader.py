...
from Lara.model.modules.action_model.flow_matching_head.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)

from Lara.model.modules.action_model.flow_matching_head.cross_attention_dit import DiT
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        self.action_horizon = config.get(
            "action_horizon",
            config.future_action_window_size + 1,
        )
        self.execution_horizon = config.get("execution_horizon", self.action_horizon)
        self.execution_loss_weight = config.get("execution_loss_weight", 1.0)
        self.prediction_loss_weight = config.get("prediction_loss_weight", 1.0)
    ...
    def predict_action(self, vl_embs: torch.Tensor, state: torch.Tensor = None) -> torch.Tensor:
        ...
        for t in range(num_steps):
            ...
            t_discretized = min(int(t_cont * self.num_timestep_buckets), self.num_timestep_buckets - 1)
