...
"""
LARA trainer is built directly on native PyTorch + Accelerate + DeepSpeed, keeping the loop explicit and easy to hack.
Conventions:
...
class VLAMTrainer(TrainerUtils):
    ...
    def _init_wandb(self):
        ...
        if self.accelerator.is_main_process:
            raise RuntimeError("W&B logging is disabled by default; enable it explicitly before calling _init_wandb().")
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        if self.accelerator.is_main_process:
            ...
            examples, _video_batch = self._get_next_batch()
    ...
    def _train_step(self, batch_vla, batch_video):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_yaml", type=str, default="scripts/config/lara_cotrain.yaml", help="Path to YAML config")
