...
export MODEL_PATH="./models/VLA-JEPA/SimplerEnv/checkpoints/VLA-JEPA-SimplerEnv.pt"
fi
...
start_service() {
  ...
  CUDA_VISIBLE_DEVICES=${gpu_id} ${lara_python} deployment/model_server/server_policy.py \
    --ckpt_path ${ckpt_path} \
    --port ${port} \
    --use_bf16 \
    > "${svc_log}" 2>&1 &
