...
- Stage-2 MoE/router scaffold with residual token experts, optional direct action-chunk experts, optional routed direct-expert action output, posterior responsibility from latent tokens or per-expert action reconstruction losses, LeRobot trajectory ids for episode-level resident pool targets, reusable episode-level resident pool masks, chunk-level top-k routing inside the resident pool, optional balance/stickiness stabilizers, and route collapse diagnostics (`use_lara_moe: false` by default).
